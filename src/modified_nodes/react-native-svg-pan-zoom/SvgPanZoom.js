...
export default class SvgPanZoom extends Component {
  ...
  constructor(props) {
    ...
    this.processPinch = (x1, y1, x2, y2) => {
      ...
      const zoomScale =
        touchZoom * initialTransform.scaleX / viewTransform.scaleX;
      const panOffset = {
      ...
      const newTransform = getBoundedPinchTransform(
        viewTransform,
    ...
    const vt = this.getInitialViewTransform(
      props.canvasWidth,
      props.canvasHeight,      
      props.initialZoom
    );
  ...
  /**
   * Calculates the center of the view and renders the canvas at the center of the screen
   * @param {*} canvasWidth // the widht of canvas
   * @param {*} canvasHeight  // the height of canvas
   * @param {*} scale // the initial zoom 
   */
  getInitialViewTransform(canvasWidth, canvasHeight, scale) {
    let adjustWidth = (width/2-((canvasWidth*scale)/2));
    let adjustHeight = (height/2-((canvasHeight*scale)/2));
    let test = viewTransformMult(
      createTranslationMatrix(
        -((canvasWidth - canvasWidth * scale) / 2 - adjustWidth),
        -((canvasHeight - canvasHeight * scale) / 2 - adjustHeight) 
      ),
